...
export default function AuditDashboard({ reports, refreshDb, onRequestConsultation }: AuditDashboardProps) {
  ...
  // SERP Simulation states
  const [customTitle, setCustomTitle] = useState<string | null>(null);
  const [customDesc, setCustomDesc] = useState<string | null>(null);
  const [isOptimizingSERP, setIsOptimizingSERP] = useState(false);
  ...
  useEffect(() => {
    ...
    // Reset custom SERP metadata optimization variations on selection change
    setCustomTitle(null);
    setCustomDesc(null);
    setIsOptimizingSERP(false);
          ...
          <div className="lg:col-span-8 space-y-6">
            {activeReport && (
              <div className="space-y-6">
                ...
                {/* Google Search Live SERP Snippet Previewer */}
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-850 pb-3">
                    <div className="flex items-center gap-2">
                      <Globe className="w-5 h-5 text-emerald-400" />
                      <div>
                        <h4 className="text-xs font-semibold text-white uppercase tracking-widest leading-none">Google SERP Simulator</h4>
                        <p className="text-[10px] text-slate-400 mt-1">Simulate click metadata appearance under search engine results page</p>
                      </div>
                    </div>
                    <span className="text-[10px] font-mono text-[#10b981] bg-emerald-950/40 border border-emerald-900/30 px-1.5 py-0.5 rounded">SEO Live</span>
                  </div>

                  {/* Simulated Google Search Result */}
                  <div className="bg-white p-5 rounded-xl border border-slate-200 text-slate-900 space-y-2 max-w-full overflow-hidden shadow-sm">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center font-bold text-[#1a0dab] border border-slate-200 text-[10px] select-none">
                        G
                      </div>
                      <div className="space-y-0.5 min-w-0">
                        <p className="text-[11px] text-[#202124] font-sans leading-none truncate flex items-center gap-1 font-medium">
                          {activeReport.url.replace(/https?:\/\//i, "")} <span className="text-[9px] text-[#70757a] font-normal">› search</span>
                        </p>
                        <p className="text-[9px] text-[#5f6368] leading-none truncate select-all">https://{activeReport.url.replace(/https?:\/\/(www\.)?/i, "")}</p>
                      </div>
                    </div>

                    {/* Blue Title Link */}
                    <h5 className="text-lg text-[#1a0dab] hover:underline font-sans font-medium leading-tight cursor-pointer break-words">
                      {customTitle ? (
                        customTitle
                      ) : activeReport.checks.metaTagsPresent ? (
                        `${activeReport.url.replace(/^(https?:\/\/)?(www\.)?/, "").replace(/\..*$/, "").replace(/^\w/, c => c.toUpperCase())} | Core Metrics & Optimization`
                      ) : (
                        `Index of /${activeReport.url.replace(/^(https?:\/\/)?(www\.)?/, "").replace(/\..*$/, "")}`
                      )}
                    </h5>

                    {/* Search Snippet */}
                    <p className="text-xs text-[#4d5156] font-sans leading-relaxed break-words">
                      <span className="text-[#70757a] select-none text-[11px] font-semibold">{new Date(activeReport.scannedAt).toLocaleDateString()} — </span> 
                      {customDesc ? (
                        customDesc
                      ) : activeReport.checks.metaTagsPresent ? (
                        `Experience high-converting enterprise performance scores for ${activeReport.url.replace(/^(https?:\/\/)?(www\.)?/, "")}. View responsive web layouts, chatbot widgets, and customer leads.`
                      ) : (
                        `Warning: Primary metadata is missing on ${activeReport.url.replace(/^(https?:\/\/)?(www\.)?/, "")}. Generate high CTR descriptors and page speed reports immediately to gain organic visitor visibility.`
                      )}
                    </p>
                  </div>
                  ...
                  <div className="bg-slate-950/60 border border-slate-850 p-4 rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-1">
                      <p className="text-[10px] font-bold text-yellow-400 uppercase tracking-wider flex items-center gap-1">
                        <Sparkles className="w-3.5 h-3.5" /> High-conversion Meta Tags
                      </p>
                      <p className="text-[11px] text-slate-400">
                        Compute optimal headlines & descriptions leveraging client-focused keyword indexing schemas.
                      </p>
                    </div>
                    ...
                    <div className="flex gap-2 self-end md:self-auto">
                      {(customTitle || customDesc) && (
                        <button
                          ...
                          className="px-3 py-2 bg-slate-900 border border-slate-800 hover:border-slate-700 rounded-lg text-[10px] font-bold text-slate-400 hover:text-slate-200 transition-all cursor-pointer"
                        >
                          Reset Title
                        </button>
                      )}
                      ...
                      <button
                        type="button"
                        disabled={isOptimizingSERP}
                        onClick={() => {
                          setIsOptimizingSERP(true);
                          setTimeout(() => {
                            const cleanDomain = activeReport.url.replace(/^(https?:\/\/)?(www\.)?/, "").replace(/\..*$/, "").replace(/^\w/, c => c.toUpperCase());
                            setCustomTitle(`🚀 ${cleanDomain}™ - Modern AI Workflows & Digital Optimization App`);
                            setCustomDesc(`Discover top-tier performance for ${cleanDomain}! We build live custom customer support agents, repair page latency indexes, and capture CRM audience leads instantly. Book a consultation today.`);
                            setIsOptimizingSERP(false);
                          }, 900);
                        }}
                        className="px-3.5 py-2 bg-slate-800 hover:bg-slate-755 text-[10px] font-bold text-white transition-all cursor-pointer rounded-lg flex items-center gap-1 shadow-sm"
                      >
                        {isOptimizingSERP ? (
                          <>
